...
def more_pour_problem(capacities, goal, start=None):
    ...
    def successors(state):
        ...
        for index in range(len(capacities)):
            ...
            new_state[index] = capacities[index]
            d[tuple(new_state)] = action
            ...
            for index2 in range(len(capacities)):
                if index2 != index:
                    action = ('pour', index, index2)
                    new_state = list(state)

                    if state[index2] + state[index] < capacities[index2]:
                        new_state[index] = 0
                        new_state[index2] = state[index2] + state[index]
                    else:
                        new_state[index] = state[index] - (capacities[index2] - state[index2])
                        new_state[index2] = capacities[index2]
                    d[tuple(new_state)] = action
        return d

    if start == None: start = tuple([0 for i in range(len(capacities))])
